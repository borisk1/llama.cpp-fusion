...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--thread-copy"}, "RANGES",
        "comma-separated cpu core ranges for thread-copy groups (e.g. \"0-7,8-15\").\n"
        "each group gets its own copy of model weights and KV cache, pinned to its cores.\n"
        "a hierarchical barrier keeps per-op sync from becoming the bottleneck.",
        [](common_params & params, const std::string & spec) {
            auto fields = string_split<std::string>(spec, ',');
            if (fields.size() < 2) {
                throw std::invalid_argument("--thread-copy requires at least 2 groups (comma-separated ranges)");
            }
            if (fields.size() > COMMON_CPU_COPY_MAX_GROUPS) {
                throw std::invalid_argument("--thread-copy: too many groups (max " +
                    std::to_string(COMMON_CPU_COPY_MAX_GROUPS) + ")");
            }
            params.cpuparams.thread_copy_groups.clear();
            for (const auto & f : fields) {
                common_cpu_copy_group g;
                g.range = f;
                if (!parse_cpu_range(f, g.cpumask)) {
                    throw std::invalid_argument("--thread-copy: invalid cpu range '" + f + "'");
                }
                g.n_threads = 0;
                for (int i = 0; i < GGML_MAX_N_THREADS; i++) {
                    if (g.cpumask[i]) g.n_threads++;
                }
                if (g.n_threads == 0) {
                    throw std::invalid_argument("--thread-copy: empty cpu range '" + f + "'");
                }
                params.cpuparams.thread_copy_groups.push_back(std::move(g));
            }
            if (!params.cpuparams.thread_copy_groups.empty()) {
                params.cpuparams.n_threads = 0;
                for (const auto & g : params.cpuparams.thread_copy_groups) {
                    params.cpuparams.n_threads += g.n_threads;
                }
            }
        }
    ));
    ...
    add_opt(common_arg(
        {"--profile-tensors"},
        "profile per-tensor CPU/GPU execution time for optimal placement",
        [](common_params & params) {
