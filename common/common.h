...
// a set of cpu core ranges defining a single thread-copy group
struct common_cpu_copy_group {
    std::string range;                          // e.g. "0-7"
    bool cpumask[GGML_MAX_N_THREADS] = {false}; // parsed mask
    int n_threads = 0;                          // number of cores in the mask
    int numa_node = -1;                         // NUMA node for this group (-1 = not detected)
};

#define COMMON_CPU_COPY_MAX_GROUPS 8
...
struct common_cpu_params {
    ...
    // thread-copy: replicate weights + KV cache across core groups for higher decode throughput
    std::vector<common_cpu_copy_group> thread_copy_groups;
};
