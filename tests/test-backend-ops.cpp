...
// like init_mul_mat_id_tensors, but with DUPLICATE expert ids per token
// (hot/cold residency splits route several slots to one dummy expert)
static void init_mul_mat_id_tensors_dup(ggml_context * ctx, int n_mats) {
    std::random_device rd;
    std::default_random_engine rng(rd());
    for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
        if (t->type == GGML_TYPE_I32) {
            ...
            for (int64_t r = 0; r < ggml_nrows(t); r++) {
                std::vector<int32_t> data(t->ne[0]);
                for (int i = 0; i < t->ne[0]; i++) {
                    // ~half the slots collapse onto expert 0
                    data[i] = (i % 2 == 0) ? 0 : (i % n_mats);
                }
                std::shuffle(data.begin(), data.end(), rng);
                ggml_backend_tensor_set(t, data.data(), r * t->nb[1], t->ne[0] * sizeof(int32_t));
            }
...
// GGML_OP_MUL_MAT_ID with duplicate expert ids per token (hot/cold residency pattern)
struct test_mul_mat_id_dup : public test_mul_mat_id {
    test_mul_mat_id_dup(ggml_type type_a, ggml_type type_b, int n_mats, int n_used, bool b,
                        int64_t m, int64_t n, int64_t k)
        : test_mul_mat_id(type_a, type_b, n_mats, n_used, b, m, n, k) {}

    std::string op_desc(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return "MUL_MAT_ID_DUP";
    }

    void initialize_tensors(ggml_context * ctx) override {
        init_mul_mat_id_tensors_dup(ctx, n_mats);
    }
};
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // hot-resident residency shapes: many experts, qwen35moe-like dims,
    // both unique and DUPLICATE ids per token
    for (int n_mats : {64, 72, 256}) {
        for (bool b : {false, true}) {
            for (int n : {1, 2, 4}) {
                test_cases.emplace_back(new test_mul_mat_id    (GGML_TYPE_Q4_K, GGML_TYPE_F32, n_mats, 8, b, 768, n, 2048));
                test_cases.emplace_back(new test_mul_mat_id_dup(GGML_TYPE_Q4_K, GGML_TYPE_F32, n_mats, 8, b, 768, n, 2048));
            }
        }
    }
