...
static struct gguf_context * gguf_init_from_reader(const struct gguf_reader & gr, struct gguf_init_params params) {
    ...
    for (int64_t i = 0; ok && i < n_tensors; ++i) {
        ...
        // tensor type
        {
            ...
            // DS4 GGUF: allow custom type 101 (Q4RMFP4, 4-bit row-wise mixed FP4)
            if (info.t.type == 101) {
                // treated as 4-bit quant with 32-element blocks
                // type_size = 20 bytes per 32-element block (16 bytes data + 4 bytes scale)
            } else if (info.t.type < 0 || info.t.type >= GGML_TYPE_COUNT) {
                GGML_LOG_ERROR("%s: tensor '%s' has invalid ggml type %d. should be in [0, %d)\n",
                    __func__, info.t.name, info.t.type, GGML_TYPE_COUNT);
                ok = false;
                break;
            }
            const size_t  type_size = (info.t.type == 101) ? 17 : ggml_type_size(info.t.type);
            const int64_t blck_size = (info.t.type == 101) ? 32 : ggml_blck_size(info.t.type);
    ...
    // compute the total size of the data section, taking into account the alignment
    {
        ...
        for (size_t i = 0; i < ctx->info.size(); ++i) {
            ...
            if (ti.offset != ctx->size) {
                // DS4 compatibility: Q4RMFP4 (type 101) has ~17 bytes/block but
                // GGML internal type_size can't account for alignment differences
                if (ti.t.type == 101) {
                    ctx->size = ti.offset;
                } else {
