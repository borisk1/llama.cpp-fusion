...
struct ggml_threadpool {
    ...
    // hierarchical barrier for thread-copy groups
    int n_groups;  // 1 = normal mode, >1 = thread-copy mode
    int group_leader_ith[GGML_MAX_COPY_GROUPS]; // ith of the leader thread for each group (typically 0 for each group, but indexed globally)
    atomic_int GGML_CACHE_ALIGN group_n_barrier      [GGML_MAX_COPY_GROUPS];
    atomic_int GGML_CACHE_ALIGN group_n_barrier_passed[GGML_MAX_COPY_GROUPS];
    atomic_int GGML_CACHE_ALIGN n_barrier_global;
    atomic_int GGML_CACHE_ALIGN n_barrier_passed_global;

    bool use_hierarchical_barrier; // set false when CUDA present to avoid deadlock
...
struct ggml_compute_state {
    ...
    int ith;
    int group_id; // thread-copy group id (0 based, -1 = not in a group)
};
...
// group-local barrier: only syncs threads that belong to the same group
// must be called with group_id == workers[ith].group_id
static void ggml_barrier_group(struct ggml_threadpool * tp, int group_id) {
    if (tp->n_groups <= 1) {
        ggml_barrier(tp);
        return;
    }

    int n_threads_active = atomic_load_explicit(&tp->n_graph, memory_order_relaxed) & GGML_THREADPOOL_N_THREADS_MASK;
    int n_group_threads = 0;
    for (int i = 0; i < tp->n_threads && i < n_threads_active; i++) {
        if (tp->workers[i].group_id == group_id) {
            n_group_threads++;
        }
    }
    if (n_group_threads <= 1) {
        return;
    }

    int gi = group_id;
    if (gi < 0 || gi >= GGML_MAX_COPY_GROUPS) {
        return;
    }

    int n_passed = atomic_load_explicit(&tp->group_n_barrier_passed[gi], memory_order_relaxed);

    // enter barrier
    int n_barrier = atomic_fetch_add_explicit(&tp->group_n_barrier[gi], 1, memory_order_seq_cst);

    if (n_barrier == (n_group_threads - 1)) {
        // last thread in group
        atomic_store_explicit(&tp->group_n_barrier[gi], 0, memory_order_relaxed);
        atomic_fetch_add_explicit(&tp->group_n_barrier_passed[gi], 1, memory_order_seq_cst);
        return;
    }

    while (atomic_load_explicit(&tp->group_n_barrier_passed[gi], memory_order_relaxed) == n_passed) {
        ggml_thread_cpu_relax();
    }

    #ifdef GGML_TSAN_ENABLED
    atomic_fetch_add_explicit(&tp->group_n_barrier_passed[gi], 0, memory_order_seq_cst);
    #else
    atomic_thread_fence(memory_order_seq_cst);
    #endif
}

// global barrier across groups: only group leader threads participate
static void ggml_barrier_global(struct ggml_threadpool * tp) {
    if (tp->n_groups <= 1) {
        return;
    }

    int n_passed = atomic_load_explicit(&tp->n_barrier_passed_global, memory_order_relaxed);

    int n_barrier = atomic_fetch_add_explicit(&tp->n_barrier_global, 1, memory_order_seq_cst);

    if (n_barrier == (tp->n_groups - 1)) {
        atomic_store_explicit(&tp->n_barrier_global, 0, memory_order_relaxed);
        atomic_fetch_add_explicit(&tp->n_barrier_passed_global, 1, memory_order_seq_cst);
        return;
    }

    while (atomic_load_explicit(&tp->n_barrier_passed_global, memory_order_relaxed) == n_passed) {
        ggml_thread_cpu_relax();
    }

    #ifdef GGML_TSAN_ENABLED
    atomic_fetch_add_explicit(&tp->n_barrier_passed_global, 0, memory_order_seq_cst);
    #else
    atomic_thread_fence(memory_order_seq_cst);
    #endif
}
    ...
    if (ith == 0) {
            ...
            if (moe_cache_dev >= 0) {
                ...
                for (int64_t iid1 = 0; iid1 < ids->ne[1]; ++iid1) {
                    for (int id = 0; id < n_ids; ++id) {
                        ...
                        // hot-resident interop: out-of-range sentinel ids (dual-chain
                        // cold path masks hot slots with id >= n_as) must never reach
                        // the cache planner — map them to -1 (guaranteed miss, never
                        // an insert candidate)
                        if (cid < 0 || cid >= n_as) {
                            cid = -1;
                        }
                        moe_cache_ids[iid1*n_ids + id] = cid;
                    }
                }
                ggml_moe_cache.plan(moe_cache_dev, moe_cache_ids, (int)(n_ids * ids->ne[1]), moe_cache_slot_idx);
                // enforce miss for masked slots regardless of planner behavior
        ...
        for (int64_t iid1 = 0; iid1 < ids->ne[1]; ++iid1) {
            for (int id = 0; id < n_ids; ++id) {
                ...
                // out-of-range expert ids mark slots routed to another
                // residency path (hot/cold split): skip compute, zero output
                if (i02 < 0 || i02 >= n_as) {
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    // thread-copy: create stack-local tensor copies with group-adjusted data pointers
    struct ggml_threadpool * tp = params->threadpool;
    struct ggml_tensor local_tensor;
    struct ggml_tensor local_src[GGML_MAX_SRC];
    struct ggml_tensor * local_src_ptrs[GGML_MAX_SRC];

    if (tp && tp->n_groups > 1) {
        int gid = tp->workers[params->ith].group_id;
        if (gid > 0) {  // group 0 uses original data pointers (no adjustment)
            bool has_group_data = (tensor->data_numa != NULL);
            for (int i = 0; i < GGML_MAX_SRC && !has_group_data; i++) {
                if (tensor->src[i] && tensor->src[i]->data_numa) {
                    has_group_data = true;
                }
            }

            if (has_group_data) {
                struct ggml_numa_mirror * m;

                // copy and adjust each source tensor that has group copies
                for (int i = 0; i < GGML_MAX_SRC; i++) {
                    struct ggml_tensor * src = tensor->src[i];
                    if (src && src->data_numa) {
                        local_src[i] = *src;
                        m = (struct ggml_numa_mirror *)src->data_numa;
                        local_src[i].data = m->data[gid];
                        local_src_ptrs[i] = &local_src[i];
                    } else {
                        local_src_ptrs[i] = src;
                    }
                }

                // copy the tensor struct with adjusted src pointers
                local_tensor = *tensor;
                memcpy(local_tensor.src, local_src_ptrs, sizeof(local_tensor.src));

                // adjust tensor's own data if it has group copies
                if (tensor->data_numa) {
                    m = (struct ggml_numa_mirror *)tensor->data_numa;
                    local_tensor.data = m->data[gid];
                }
...
static int ggml_get_n_tasks(struct ggml_tensor * node, int n_threads) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_LIGHTNING_INDEXER:
            {
                n_tasks = MIN((int)node->src[1]->ne[2], n_threads);
            } break;
        case GGML_OP_NONE:
            {
                n_tasks = 1;
                ...
                case GGML_OP_LIGHTNING_INDEXER:
                    {
                        ...
                        cur = (int64_t)(n_embd + CACHE_LINE_SIZE_F32) * sizeof(float) * n_tasks;
                    } break;
                case GGML_OP_COUNT:
                    {
                        GGML_ABORT("fatal error");
...
static thread_ret_t ggml_graph_compute_thread(void * data) {
    ...
    const bool use_hierarchical = (tp->n_groups > 1 && tp->use_hierarchical_barrier);
    ...
    for (int node_n = 0; node_n < cgraph->n_nodes && atomic_load_explicit(&tp->abort, memory_order_relaxed) != node_n; node_n++) {
        ...
        if (node_n + 1 < cgraph->n_nodes) {
            if (use_hierarchical) {
                int gid = state->group_id;
                if (gid < 0) gid = 0;
                ggml_barrier_group(tp, gid);
                if (state->ith == tp->group_leader_ith[gid]) {
                    ggml_barrier_global(tp);
                }
                ggml_barrier_group(tp, gid);
            } else {
...
static thread_ret_t ggml_graph_compute_secondary_thread(void* data) {
    ...
    while (true) {
        ...
        if (state->pending) {
            ...
            // re-apply affinity in case the cpumask was updated (e.g. thread-copy groups)
            if (ggml_thread_cpumask_is_valid(state->cpumask)) {
                ggml_thread_apply_affinity(state->cpumask);
            }
...
static struct ggml_threadpool * ggml_threadpool_new_impl(
        ...
        ggml_aligned_malloc(sizeof(struct ggml_threadpool));
    {
        ...
        // init group fields to defaults
        threadpool->n_groups = 1;
        threadpool->use_hierarchical_barrier = true;
        for (int gi = 0; gi < GGML_MAX_COPY_GROUPS; gi++) {
            threadpool->group_leader_ith[gi]               = -1;
        }
        threadpool->n_barrier_global        = 0;
        threadpool->n_barrier_passed_global = 0;
    }
...
void ggml_threadpool_set_thread_copy_groups(
        struct ggml_threadpool * tp,
        int n_groups,
        const bool group_cpumask[][GGML_MAX_N_THREADS],
        const int group_n_threads[]) {

    if (tp == NULL || n_groups <= 1) {
        return;
    }
    if (n_groups > GGML_MAX_COPY_GROUPS) {
        n_groups = GGML_MAX_COPY_GROUPS;
    }

    tp->n_groups = n_groups;
    int total_group_threads = 0;
    for (int gi = 0; gi < n_groups; gi++) {
        total_group_threads += group_n_threads[gi];
    }

    if (total_group_threads > tp->n_threads) {
        total_group_threads = tp->n_threads;
    }

    // sequential assignment: first group_n_threads[0] workers -> group 0,
    // next group_n_threads[1] -> group 1, etc.
    int worker_idx = 0;
    for (int gi = 0; gi < n_groups; gi++) {
        tp->group_leader_ith[gi] = -1;
        int assigned = 0;
        while (worker_idx < tp->n_threads && assigned < group_n_threads[gi]) {
            tp->workers[worker_idx].group_id = gi;
            if (tp->group_leader_ith[gi] < 0) {
                tp->group_leader_ith[gi] = worker_idx;
            }
            assigned++;
            worker_idx++;
        }

        // overwrite this worker's cpumask with the group-specific mask for NUMA pinning
        for (int wi = 0; wi < tp->n_threads; wi++) {
            if (tp->workers[wi].group_id == gi) {
                memcpy(tp->workers[wi].cpumask, group_cpumask[gi], sizeof(tp->workers[wi].cpumask));
            }
        }
    }

    // apply group-specific cpumask for main thread (worker 0)
    if (tp->workers[0].group_id >= 0 && ggml_thread_cpumask_is_valid(tp->workers[0].cpumask)) {
        ggml_thread_apply_affinity(tp->workers[0].cpumask);
    }

    // reinitialize group barrier atomics
    for (int gi = 0; gi < n_groups; gi++) {
        atomic_store_explicit(&tp->group_n_barrier      [gi], 0, memory_order_relaxed);
        atomic_store_explicit(&tp->group_n_barrier_passed[gi], 0, memory_order_relaxed);
    }
    atomic_store_explicit(&tp->n_barrier_global,        0, memory_order_relaxed);
    atomic_store_explicit(&tp->n_barrier_passed_global, 0, memory_order_relaxed);
}

int ggml_threadpool_get_group_id(struct ggml_threadpool * tp, int ith) {
    if (tp == NULL || ith < 0 || ith >= tp->n_threads) return -1;
    return tp->workers[ith].group_id;
}

int ggml_threadpool_get_n_groups(struct ggml_threadpool * tp) {
    return tp ? tp->n_groups : 1;
}

void ggml_threadpool_set_n_groups(struct ggml_threadpool * tp, int n_groups) {
    if (tp) tp->n_groups = n_groups;
}

void ggml_threadpool_set_hierarchical_barrier(struct ggml_threadpool * tp, bool enable) {
    if (tp) tp->use_hierarchical_barrier = enable;
}
