...
struct ggml_backend_sched {
    ...
    // async CPU split execution (GGML_SCHED_ASYNC_CPU=1), lazily created
    struct ggml_sched_async_ctx * async_ctx;
};
...
// ===================================================================
// Async CPU split execution (GGML_SCHED_ASYNC_CPU=1)
//
// Motivation (hot-resident dual-chain MoE): per layer the graph contains a
// CPU split (cold-expert chain) whose result is consumed only by a join add.
// The stock scheduler computes CPU splits synchronously on the calling
// thread, so the GPU idles while the CPU crunches cold experts. With this
// extension, CPU splits are pushed to a single worker thread (FIFO) and the
// scheduler keeps issuing subsequent GPU splits (e.g. the hot-expert chain,
// which has no CPU inputs). A later split that consumes a CPU-produced
// tensor drains the worker queue before copying its inputs.
//
// Pairs with a name-based split break: nodes named "hr_join*" start a new
// split so the hot chain is issued to the GPU BEFORE the drain point.
// ===================================================================
#include <thread>
#include <mutex>
...
struct ggml_sched_async_ctx {
    std::thread th;
    std::mutex mtx;
    std::condition_variable cv_push, cv_done;
    std::deque<std::pair<ggml_backend_t, struct ggml_cgraph *>> q;
    int pending = 0;
    bool stop = false;
    enum ggml_status err = GGML_STATUS_SUCCESS;

    ggml_sched_async_ctx() {
        th = std::thread([this]() {
            for (;;) {
                std::pair<ggml_backend_t, struct ggml_cgraph *> job;
                {
                    std::unique_lock<std::mutex> lk(mtx);
                    cv_push.wait(lk, [this]{ return stop || !q.empty(); });
                    if (stop && q.empty()) return;
                    job = q.front();
                    q.pop_front();
                }
                enum ggml_status ec = ggml_backend_graph_compute_async(job.first, job.second);
                {
                    std::lock_guard<std::mutex> lk(mtx);
                    if (ec != GGML_STATUS_SUCCESS && err == GGML_STATUS_SUCCESS) err = ec;
                    pending--;
                }
                cv_done.notify_all();
            }
    ...
    void push(ggml_backend_t backend, struct ggml_cgraph * graph) {
        {
            std::lock_guard<std::mutex> lk(mtx);
            q.emplace_back(backend, graph);
            pending++;
        }
        cv_push.notify_one();
    }
    enum ggml_status drain() {
        std::unique_lock<std::mutex> lk(mtx);
...
static bool ggml_backend_sched_async_cpu_enabled(void) {
    static int en = -1;
    if (en == -1) {
        const char * s = getenv("GGML_SCHED_ASYNC_CPU");
        en = (s && atoi(s) != 0) ? 1 : 0;
        if (en) GGML_LOG_INFO("%s: async CPU split execution enabled\n", __func__);
    }
    return en == 1;
}
...
void ggml_backend_sched_split_graph(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    // pass 5: split graph, find tensors that need to be copied
    {
        ...
        for (; i < graph->n_nodes; i++) {
            ...
            // async CPU mode: nodes named "hr_join*" (dual-chain MoE join) start
            // a new split so the preceding same-backend (GPU) hot chain is issued
            // before the scheduler blocks on the CPU-produced input of the join
            if (node_backend_id == cur_backend_id &&
                ggml_backend_sched_async_cpu_enabled() &&
                strncmp(ggml_get_name(node), "hr_join", 7) == 0) {
                need_new_split = true;
            }
...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    // async CPU split execution: lazily create the worker context
    const bool async_cpu = ggml_backend_sched_async_cpu_enabled() && !sched->callback_eval;
    const int  cpu_backend_id = sched->n_backends - 1;   // CPU is always the last backend
    if (async_cpu && sched->async_ctx == NULL) {
        sched->async_ctx = new ggml_sched_async_ctx();
    }
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        // async CPU: drain pending CPU work when (a) a non-CPU split consumes a
        // CPU-produced input (the copy path below reads CPU tensor data directly),
        // or (b) the next split is itself a CPU split (its input copies could
        // alias input copies still being read by the in-flight CPU job).
        if (async_cpu && sched->async_ctx->has_pending()) {
            bool need_drain = (split_backend_id == cpu_backend_id);
            if (!need_drain) {
                for (int input_id = 0; input_id < split->n_inputs; input_id++) {
                    ggml_backend_t input_backend = ggml_backend_sched_get_tensor_backend(sched, split->inputs[input_id]);
                    if (input_backend == sched->backends[cpu_backend_id]) {
                        need_drain = true;
                        break;
                    }
                }
            ...
            if (need_drain) {
                enum ggml_status ec = sched->async_ctx->drain();
        ...
        if (!sched->callback_eval) {
            if (async_cpu && split_backend_id == cpu_backend_id) {
                // dispatch the CPU split to the worker; the scheduler keeps
                // issuing subsequent (GPU) splits that have no CPU inputs
                static int dispatch_dbg = 0;
                if (dispatch_dbg < 3) {
                    fprintf(stderr, "[sched-async] dispatching CPU split %d to worker (n_splits=%d)\n", split_id, sched->n_splits);
                    dispatch_dbg++;
                }
                sched->async_ctx->push(split_backend, &split->graph);
            } else {
    ...
    // async CPU: all work must be complete before returning — callers read
    // outputs (logits, embeddings, post-compute readbacks) right after this
    if (async_cpu && sched->async_ctx->has_pending()) {
        enum ggml_status ec = sched->async_ctx->drain();
        if (ec != GGML_STATUS_SUCCESS) {
            return ec;
...
void ggml_backend_sched_free(ggml_backend_sched_t sched) {
    ...
    if (sched->async_ctx != NULL) {
        sched->async_ctx->drain();
        delete sched->async_ctx;
        sched->async_ctx = NULL;
    }
    for (int b = 0; b < sched->n_backends; b++) {
        for (int c = 0; c < sched->n_copies; c++) {
...
void ggml_backend_sched_synchronize(ggml_backend_sched_t sched) {
    GGML_ASSERT(sched);
    if (sched->async_ctx != NULL) {
        sched->async_ctx->drain();   // async CPU work must finish before backend syncs
    }
    for (int i = 0; i < sched->n_backends; i++) {
        ggml_backend_synchronize(sched->backends[i]);
    }
