...
static void ggml_cuda_mul_mat_cublas_impl(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (compute_type == GGML_TYPE_F32 && ne12 == 1 && ne13 == 1) {
        // forensic guard: cublasSgemm rejects these silently as "invalid value" —
        // print full tensor context first so the culprit op is identifiable
        static int log_sgemm = -1;
        if (log_sgemm == -1) {
            const char * s = getenv("GGML_CUDA_LOG_SGEMM");
            log_sgemm = (s && atoi(s) != 0) ? 1 : 0;
        }
        if (log_sgemm == 1 ||
            ne01 <= 0 || ne10 <= 0 || ne11 <= 0 || s01 < ne10 || s11 < ne10 || ne0 < ne01 ||
            src0_ptr == nullptr || src1_ptr == nullptr || dst_ptr == nullptr) {
            GGML_LOG_ERROR("%s: Sgemm m=%lld n=%lld k=%lld lda=%lld ldb=%lld ldc=%lld "
                           "src0='%s'(%s) src1='%s'(%s) dst='%s' stream_no=%d handle=%p ptrs=%p,%p,%p\n",
                __func__,
                (long long)ne01, (long long)ne11, (long long)ne10, (long long)s01, (long long)s11, (long long)ne0,
                src0->name, ggml_type_name(src0->type), src1->name, ggml_type_name(src1->type), dst->name,
                ctx.curr_stream_no, (void *) ctx.cublas_handle(),
                (const void *)src0_ptr, (const void *)src1_ptr, (const void *)dst_ptr);
        }
        CUBLAS_CHECK(
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // Hot/cold expert residency splits mark foreign-path slots with
    // out-of-range expert ids; those slots are skipped by every kernel below,
    // so their dst rows must be zeroed explicitly. Env-gated: zero overhead
    // in normal operation.
    static const bool mmid_zero_dst = getenv("GGML_MMID_ZERO_DST") != nullptr;
    if (mmid_zero_dst) {
    ...
    static const bool mmid_dbg = getenv("GGML_MMID_DEBUG") != nullptr;
    ...
    if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        if (ne2 <= MMVQ_MAX_BATCH_SIZE) {
            if (ggml_is_quantized(src0->type)) {
                ...
                if (ne2 <= mmvq_mmid_max) {
                    if (mmid_dbg) fprintf(stderr, "[mmid] %s: mmvq path (ne2=%d n_as=%d)\n", dst->name, (int)ne2, (int)ne02);
        ...
        if (ggml_cuda_should_use_mmq(src0->type, cc, ne12, /*n_experts=*/ne02)) {
            if (mmid_dbg) fprintf(stderr, "[mmid] %s: mmq path (ne2=%d n_as=%d)\n", dst->name, (int)ne2, (int)ne02);
            ggml_cuda_mul_mat_q(ctx, src0, src1, ids, dst);
            return;
        }

        if (ggml_cuda_should_use_mmf(src0->type, cc, WARP_SIZE, src0->ne, src0->nb, src1->ne[2], /*mul_mat_id=*/true)) {
            if (mmid_dbg) fprintf(stderr, "[mmid] %s: mmf path (ne2=%d n_as=%d)\n", dst->name, (int)ne2, (int)ne02);
            ggml_cuda_mul_mat_f(ctx, src0, src1, ids, dst);
            return;
        }
    }
    if (mmid_dbg) fprintf(stderr, "[mmid] %s: FALLBACK path (ne2=%d n_as=%d)\n", dst->name, (int)ne2, (int)ne02);
...
ggml_backend_t ggml_backend_cuda_init(int device) {
    ...
    // Eagerly initialize cuBLAS while VRAM is still available. The lazy path
    // (first cublas call happening at runtime — e.g. a prompt batch routed
    // through a speculative-draft context on a nearly-full device) makes
    // cuBLAS fail its internal workspace allocation and surface it as a
    // confusing CUBLAS_STATUS_INVALID_VALUE mid-inference. Warming it up at
    // init time turns that failure mode into a load-time warning.
    {
        ggml_cuda_set_device(device);
        cublasHandle_t handle = ctx->cublas_handle(device);
        float * d = nullptr;
        if (cudaMalloc((void **) &d, 3 * sizeof(float)) == cudaSuccess) {
            const float alpha = 1.0f, beta = 0.0f;
            CUDA_CHECK(cudaMemsetAsync(d, 0, 3 * sizeof(float), ctx->stream()));
            cublasSetStream(handle, ctx->stream());
            cublasStatus_t st = cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, 1, 1, 1,
                                            &alpha, d, 1, d + 1, 1, &beta, d + 2, 1);
            cudaStreamSynchronize(ctx->stream());
            cudaFree(d);
            if (st != CUBLAS_STATUS_SUCCESS) {
                GGML_LOG_WARN("%s: eager cuBLAS warm-up failed on device %d (status %d) — "
                              "device likely low on memory; cuBLAS ops may fail at runtime\n",
                              __func__, device, (int) st);
            }
        } else {
            GGML_LOG_WARN("%s: eager cuBLAS warm-up skipped on device %d (no memory for scratch)\n",
                          __func__, device);
        }
    }
