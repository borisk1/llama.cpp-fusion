...
bool ggml_cuda_should_use_mmvf(enum ggml_type type, int cc, const int64_t * src0_ne, const size_t * src0_nb, int64_t ne11) {
    ...
    switch (type) {
        case GGML_TYPE_F32:
            // single-row src0 (e.g. shared-expert gates: [k, 1]) — always prefer the
            // vector kernel over cuBLAS: an m=1 SGEMM has pure per-call overhead on
            // the cuBLAS side, and its lazy internal workspace allocation can fail
            // (CUBLAS_STATUS_INVALID_VALUE) on a nearly-full device mid-inference,
            // e.g. with speculative verify batches (ne11 2-4) under tight VRAM.
            if (src0_ne[1] == 1) {
                return ne11 <= 8;
            }
