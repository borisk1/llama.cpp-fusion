...
bool ggml_is_contiguous_to_3(const struct ggml_tensor * tensor) {
    // Check dimensions 0,1,2 are contiguous (nb matches expected stride)
    size_t next_nb = ggml_type_size(tensor->type);
    if (tensor->ne[0] != ggml_blck_size(tensor->type) && tensor->nb[0] != next_nb) {
        return false;
    }
    next_nb *= tensor->ne[0]/ggml_blck_size(tensor->type);
    for (int i = 1; i <= 2 && i < GGML_MAX_DIMS; i++) {
...
//
// NUMA-aware allocation for thread-copy / NUMA mirror
//
...
#define GGML_MPOL_BIND 2
#define GGML_NUMA_HUGE_ALIGN ((size_t)(2u << 20))
...
void * ggml_numa_alloc(size_t size, int node) {
    ...
    if (node >= 0) {
        ...
        syscall(SYS_mbind, p, aligned, GGML_MPOL_BIND, &nodemask, sizeof(unsigned long)*8, 0);
    }
    return p;
}
...
void ggml_numa_bind(void * ptr, size_t size, int node) {
    ...
    unsigned long nodemask = 1UL << (unsigned long)node;
    syscall(SYS_mbind, (void *)aligned_start, len, GGML_MPOL_BIND, &nodemask, sizeof(unsigned long)*8, 0);
}

void ggml_numa_mirror_set(struct ggml_tensor * tensor, void * const * node_data) {
    if (!tensor || !node_data) return;
    if (!tensor->data_numa) {
        tensor->data_numa = calloc(1, sizeof(struct ggml_numa_mirror));
    }
...
void ggml_numa_mirror_resync(struct ggml_tensor * tensor) {
    if (!tensor || !tensor->data_numa) return;
    struct ggml_numa_mirror * m = (struct ggml_numa_mirror *)tensor->data_numa;
...
void ggml_numa_free(void * ptr, size_t size) { (void)size; free(ptr); }
void ggml_numa_bind(void * ptr, size_t size, int node) { (void)ptr; (void)size; (void)node; }
void ggml_numa_mirror_set(struct ggml_tensor * tensor, void * const * node_data) {
    if (!tensor || !node_data) return;
    if (!tensor->data_numa) {
        tensor->data_numa = calloc(1, sizeof(struct ggml_numa_mirror));
