...
extern "C" {
    ...
    enum ggml_type {
        ...
        GGML_TYPE_Q2_0    = 42,
        GGML_TYPE_COUNT   = 43,
        // DS4 GGUF custom quant type (Q4RMFP4 — 4-bit row-wise mixed FP4)
        GGML_TYPE_Q4RMFP4 = 101,
    };
    ...
    struct ggml_tensor {
        ...
        // thread-copy / NUMA mirror: per-node/per-group data copies
        // points to a ggml_numa_mirror struct (defined in ggml-cpu.c)
        void * data_numa;
    };
    ...
    */
    ...
    // DeepSeek V4 lightning indexer (DSA attention scoring)
...
#    endif
    ...
    #define GGML_MAX_COPY_GROUPS 8
    #define GGML_NUMA_MAX_NODES   8

    // per-node/per-group data pointers for NUMA-aware weight/KV replication
    ...
    enum ggml_numa_mirror_flags {
        ...
        GGML_NUMA_MIRROR_KV      = 1 << 1,
        GGML_NUMA_MIRROR_ALL      = GGML_NUMA_MIRROR_WEIGHTS | GGML_NUMA_MIRROR_KV,
    };

    // thread-copy coordinator: manages multiple threadpools with replicated weights+KV cache
    struct ggml_thread_copy_params {
        int  n_groups;                                    // number of copy groups
        bool cpumask  [GGML_MAX_COPY_GROUPS][GGML_MAX_N_THREADS]; // per-group cpu masks
        int  n_threads[GGML_MAX_COPY_GROUPS];             // threads per group
        int  prio;                                        // thread priority (shared)
        uint32_t poll;                                    // polling level (shared)
        bool strict_cpu;                                  // strict cpu placement (shared)
    };
    ...
    GGML_API void ggml_threadpool_set_thread_copy_groups(
        struct ggml_threadpool * tp,
        int n_groups,
        const bool group_cpumask[][GGML_MAX_N_THREADS],
        const int group_n_threads[]);

    GGML_API int ggml_threadpool_get_group_id(struct ggml_threadpool * tp, int ith);
    GGML_API int ggml_threadpool_get_n_groups(struct ggml_threadpool * tp);
    GGML_API void ggml_threadpool_set_n_groups(struct ggml_threadpool * tp, int n_groups);
    GGML_API void ggml_threadpool_set_hierarchical_barrier(struct ggml_threadpool * tp, bool enable);

    // NUMA mirror / thread-copy data replication
    ...
    GGML_API void   ggml_numa_mirror_set     (struct ggml_tensor * tensor, void * const * node_data);
    GGML_API void   ggml_numa_mirror_resync   (struct ggml_tensor * tensor);
