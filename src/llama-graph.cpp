...
void llm_graph_result::set_outputs(const llm_graph_params & params) {
    ...
    // Expert routing outputs: mark for post-compute readback
    for (auto * t : expert_routing) {
        if (t != nullptr) {
            ggml_set_output(t);
        }
    }
...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    if (selected_experts == nullptr) {
        ...
        // Protect full argsort from compute-buffer reuse: in multi-token batches
        // the ggml allocator may recycle the src[0] buffer before downstream
        // consumers finish (the view survives but its source gets clobbered).
        ggml_set_output(selected_experts->src[0]);
        ggml_build_forward_expand(gf, selected_experts->src[0]);
    }
    cb(selected_experts, "ffn_moe_topk", il);

    // experiment (LLAMA_HR_CONT_TEST=1): feed the STANDARD path contiguous ids
    // instead of the strided argsort view — isolates CUDA mmid ids-layout bugs
    if (getenv("LLAMA_HR_CONT_TEST")) {
        selected_experts = ggml_cont(ctx0, selected_experts);
        cb(selected_experts, "ffn_moe_topk_cont", il);
    }

    // hot-cache/hit-rate instrumentation: alias the final selection tensor under
    // a stable name and protect routing tensors from graph-allocator buffer
    // reuse so they can be read back after compute
    if (hot_cache_capture_enabled()) {
        cb(selection_probs, "ffn_moe_selprobs", il);
        ggml_set_output(selection_probs);
        // selected_experts is a strided view over the argsort result — flag the
        // underlying source too, otherwise its buffer can still be recycled
        ggml_set_output(selected_experts);
        for (ggml_tensor * s = selected_experts; s->view_src != nullptr; ) {
            s = s->view_src;
            ggml_set_output(s);
        }
    }
    ...
    // ---- hot-expert VRAM residency split (env LLAMA_HOT_RESIDENT) ----
    // hot path: GPU-resident top-N expert slices, cold slots redirected to a
    // dummy expert (0) and masked out. cold path: fused CPU tensors, hot slots
    // carry an out-of-range id and are skipped by the patched CPU mul_mat_id.
    ggml_tensor * hr_gate_hot = nullptr, * hr_up_hot = nullptr, * hr_down_hot = nullptr;
    ggml_tensor * hr_hot_ids = nullptr, * hr_cold_ids = nullptr, * hr_mask = nullptr;
    bool hr_active = false;
    if (!gate_up_exps && gate_exps && up_exps && down_exps &&
        selected_experts_in == nullptr && hparams.n_expert_groups <= 1 &&
        !up_exps_b && !gate_exps_b && !down_exps_b) {
        ggml_tensor * mh = nullptr, * mc = nullptr, * mk = nullptr;
        ggml_tensor * d1 = nullptr, * d2 = nullptr, * d3 = nullptr;
        if (llama_hot_resident_get(up_exps,   &hr_up_hot,   &mh, &mc, &mk) &&
            llama_hot_resident_get(gate_exps, &hr_gate_hot, &d1, &d2, &d3) &&
            llama_hot_resident_get(down_exps, &hr_down_hot, &d1, &d2, &d3)) {
            ggml_tensor * map_hot_2d  = ggml_reshape_2d(ctx0, mh, 1, n_expert);
            ggml_tensor * map_cold_2d = ggml_reshape_2d(ctx0, mc, 1, n_expert);
            ggml_tensor * mask_2d     = ggml_reshape_2d(ctx0, mk, 1, n_expert);

            // flatten ids (argsort view is strided -> cont first); per-element
            // lookup keeps get_rows' ne[2]==ne[1] contract happy for any n_tokens
            ggml_tensor * ids_flat = ggml_reshape_1d(ctx0, ggml_cont(ctx0, selected_experts),
                                                     n_expert_used * n_tokens);

            ggml_tensor * hot_f  = ggml_get_rows(ctx0, map_hot_2d,  ids_flat); // [1, n_used*T]
            ggml_tensor * cold_f = ggml_get_rows(ctx0, map_cold_2d, ids_flat);
            ggml_tensor * mask_f = ggml_get_rows(ctx0, mask_2d,     ids_flat);
            hr_mask = ggml_reshape_3d(ctx0, mask_f, 1, n_expert_used, n_tokens);

            // unique dummy ids for cold slots: NH + slot_index (NH = number of
            // real hot experts = hot_tensor->ne[2] - n_expert_used). Guarantees
            // per-token uniqueness for CUDA mmid grouping paths; dummy experts
            // are all-zero so their output contributes nothing.
            const int64_t NH_real = hr_up_hot->ne[2] - n_expert_used;
            ggml_tensor * dummy = ggml_arange(ctx0, (float) NH_real, (float) (NH_real + n_expert_used), 1.0f); // [n_used]
            ggml_tensor * hot_f2d  = ggml_reshape_2d(ctx0, hot_f,  n_expert_used, n_tokens);
            ggml_tensor * mask_f2d = ggml_reshape_2d(ctx0, mask_f, n_expert_used, n_tokens);
            // mask is a COLD mask (1=cold, 0=hot):
            // hot_id = map_hot - mask*(map_hot - dummy) → map_hot when hot, NH+s when cold
            ggml_tensor * hot_sel = ggml_sub(ctx0, hot_f2d,
                ggml_mul(ctx0, ggml_sub(ctx0, hot_f2d, dummy), mask_f2d));

            hr_hot_ids  = ggml_cast(ctx0, hot_sel, GGML_TYPE_I32);
            hr_cold_ids = ggml_cast(ctx0, ggml_reshape_2d(ctx0, cold_f, n_expert_used, n_tokens), GGML_TYPE_I32);
            cb(hr_hot_ids,  "hr_hot_ids",  il);
            cb(hr_cold_ids, "hr_cold_ids", il);
            if (getenv("LLAMA_HR_DEBUG")) {
                // protect from buffer recycling so post-compute readback sees live data
                ggml_set_output(hr_hot_ids);
                ggml_set_output(hr_cold_ids);
                ggml_set_output(selected_experts);
                for (ggml_tensor * s = selected_experts; s->view_src != nullptr; ) { s = s->view_src; ggml_set_output(s); }
            }
            hr_active = true;
        }
    }

    // Full dual-chain split (SILU+gate only): the hot chain touches only
    // GPU-resident tensors and the cold chain only CPU tensors, joined by a
    // single add. Interleaving per-matmul would make the scheduler bounce the
    // big hot slices between backends every graph (observed: 36M+44M copies
    // per layer per token).
    bool hr_done = false;
    if (hr_active && type_op == LLM_FFN_SILU && gate_exps && !weight_before_ffn &&
        !up_exps_s && !gate_exps_s && !down_exps_s &&
        (il < 0 || hparams.swiglu_clamp_exp[il] <= 1e-6f)) {
        // NODE ORDER MATTERS for async CPU overlap (GGML_SCHED_ASYNC_CPU=1):
        // cold chain is built FIRST so the scheduler dispatches the CPU split
        // to the worker, then enqueues the GPU hot chain concurrently; the
        // join ("hr_join-<il>") forces a split break so the hot chain is not
        // blocked behind the cold-output copy.

        // cold chain (CPU): hot slots carry OOR ids → skipped+zeroed by the
        // patched CPU mul_mat_id, so the CPU only does real cold-slot work.
        // NOTE: requires --no-op-offload.
        ggml_tensor * g_c = build_lora_mm_id(gate_exps, cur, hr_cold_ids, gate_exps_s);
        cb(g_c, "ffn_moe_gate_cold", il);
        ggml_tensor * u_c = build_lora_mm_id(up_exps, cur, hr_cold_ids, up_exps_s);
        cb(u_c, "ffn_moe_up_cold", il);
        ggml_tensor * a_c = ggml_swiglu_split(ctx0, g_c, u_c);
        ggml_tensor * d_c = build_lora_mm_id(down_exps, a_c, hr_cold_ids, down_exps_s);
        cb(d_c, "ffn_moe_down_cold", il);

        // hot chain (GPU, no mask needed): cold slots route to zero dummy
        // experts (unique ids NH+s per slot) → contribute exactly 0.
        ggml_tensor * g_h = ggml_mul_mat_id(ctx0, hr_gate_hot, cur, hr_hot_ids);
        cb(g_h, "ffn_moe_gate_hot", il);
        ggml_tensor * u_h = ggml_mul_mat_id(ctx0, hr_up_hot, cur, hr_hot_ids);
        cb(u_h, "ffn_moe_up_hot", il);
        ggml_tensor * a_h = ggml_swiglu_split(ctx0, g_h, u_h);
        ggml_tensor * d_h = ggml_mul_mat_id(ctx0, hr_down_hot, a_h, hr_hot_ids);
        cb(d_h, "ffn_moe_down_hot", il);

        experts = ggml_add(ctx0, d_h, d_c);
        static int hr_async_names = -1;
        if (hr_async_names == -1) {
            const char * s = getenv("GGML_SCHED_ASYNC_CPU");
            hr_async_names = (s && atoi(s) != 0) ? 1 : 0;
        }
        if (hr_async_names) {
            ggml_format_name(experts, "hr_join-%d", il);   // split-break marker
        } else {
            cb(experts, "ffn_moe_down", il);
        }
        if (getenv("LLAMA_HR_DEBUG")) {
            // keep hot/cold sub-results alive for post-compute readback
            ggml_set_output(g_h); ggml_set_output(u_h); ggml_set_output(d_h);
            ggml_set_output(d_c); ggml_set_output(experts);
        }
        hr_done = true;
    }
    ...
    cb(experts, "ffn_moe_down", il);
    if (getenv("LLAMA_HR_DEBUG")) {
        ggml_set_output(experts);   // reliable post-compute readback of the reference
    }
    } // !hr_done
