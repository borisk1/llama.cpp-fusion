...
public:
    ...
    // thread-copy: replicate KV cache tensors for n_groups
    void set_thread_copy_groups(int n_groups, const int * numa_node_ids);

    // thread-copy: broadcast newly written KV cells from group 0 to other groups
    // cell_start, n_cells: cell range to broadcast (in tokens)
    void broadcast_thread_copy_kv(uint32_t cell_start, uint32_t n_cells);
