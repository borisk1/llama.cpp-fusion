...
void llama_tensor_profiler::print_report() const {
    LLAMA_LOG_INFO("=== Tensor Profiler Report ===\n");
    LLAMA_LOG_INFO("%-50s %10s %10s %10s %12s %12s %12s\n",
    ...
    for (const auto * e : sorted) {
        ...
        LLAMA_LOG_INFO("%-50s %10.2f %10.4f %10.4f %12.4f %12.6f %12.6f\n",
                       e->name.c_str(),
                       e->size / (1024.0 * 1024.0),
                       avg_cpu, avg_gpu, e->benefit,
                       e->epd_cpu, e->epd_gpu);
    }
    LLAMA_LOG_INFO("=== End Report ===\n");
}
...
llama_tensor_profiler::placement_solution llama_tensor_profiler::solve_knapsack(double vram_budget_bytes, double pcie_bw_gbs) const {
    ...
    if (entries.empty()) {
        LLAMA_LOG_WARN("%s: no profiling data available\n", __func__);
        return {};
    }
    ...
    LLAMA_LOG_INFO("%s: selected %zu GPU tensors (%.0f MB, benefit %.2f ms, %d switches)\n",
                   __func__, result.gpu_tensors.size(), result.total_size / (1024*1024),
