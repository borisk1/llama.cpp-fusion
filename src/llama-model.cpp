...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    if (llama_hot_resident_enabled()) {
        for (size_t il = 0; il < layers.size(); il++) {
            auto & l = layers[il];
            if (l.ffn_gate_exps && l.ffn_up_exps && l.ffn_down_exps && hparams.n_expert > 0) {
                // hybrid with --n-cpu-moe: only CPU-resident (host buffer) expert
                // tensors get the dual-chain; fully GPU-resident layers keep the
                // standard single-chain path (a hot slice there would waste VRAM,
                // and the cold chain's OOR ids are only handled by the CPU mmid)
                if (l.ffn_up_exps->buffer && !ggml_backend_buffer_is_host(l.ffn_up_exps->buffer)) {
                    continue;
                }
                llama_hot_resident_register_layer((int) il, l.ffn_gate_exps, l.ffn_up_exps, l.ffn_down_exps, (int) hparams.n_expert, (int) hparams.n_expert_used);
            }
        }
        llama_hot_resident_finalize();
    }
...
bool llama_model_set_thread_copy_groups(llama_model * model, int n_groups, const int * numa_node_ids) {
    if (!model || n_groups < 2) {
        return false;
    }

    // skip replication when all groups are on the same NUMA node (no benefit)
    if (numa_node_ids) {
        bool same_node = true;
        for (int g = 1; g < n_groups; g++) {
            if (numa_node_ids[g] != numa_node_ids[0]) { same_node = false; break; }
        }
        if (same_node) {
            // just set data_numa alias to original data for all groups
            for (auto & [name, tensor] : model->tensors_by_name) {
                if (!tensor || !tensor->data || !tensor->buffer) continue;
                if (!ggml_backend_buffer_is_host(tensor->buffer)) continue;
                if (tensor->view_src) continue;

                void * node_data[GGML_NUMA_MAX_NODES] = { NULL };
                for (int g = 0; g < n_groups; g++) {
                    node_data[g] = tensor->data; // all groups share same data
                }
                ggml_numa_mirror_set(tensor, node_data);
            }
            return true; // aliased, no memory overhead
        }
    }

    // per-buffer replication: group tensors by backend buffer, replicate buffers
    std::map<ggml_backend_buffer_t, std::vector<ggml_tensor *>> buf_tensors;
    std::map<ggml_backend_buffer_t, void *> buf_node_data[GGML_NUMA_MAX_NODES];
    std::map<ggml_backend_buffer_t, size_t> buf_sizes;

    for (auto & [name, tensor] : model->tensors_by_name) {
        if (!tensor || !tensor->data || !tensor->buffer) continue;
        if (!ggml_backend_buffer_is_host(tensor->buffer)) continue;
        // skip views - they share their parent's buffer
        if (tensor->view_src) continue;

        auto * buf = tensor->buffer;
        buf_tensors[buf].push_back(tensor);
        if (buf_sizes.find(buf) == buf_sizes.end()) {
            buf_sizes[buf] = ggml_backend_buffer_get_size(buf);
        }
    }

    if (buf_tensors.empty()) return false;

    // replicate each host buffer
    for (auto & [buf, tensors] : buf_tensors) {
        size_t buf_size = buf_sizes[buf];
        void * base = ggml_backend_buffer_get_base(buf);
        if (!base || buf_size == 0) continue;

        // group 0 reuses original buffer
        buf_node_data[0][buf] = base;
        ggml_numa_bind(base, buf_size, numa_node_ids ? numa_node_ids[0] : -1);

        bool ok = true;
        for (int g = 1; g < n_groups; g++) {
            int node = (numa_node_ids ? numa_node_ids[g] : -1);
            void * p = ggml_numa_alloc(buf_size, node);
            if (!p) { ok = false; break; }
            memcpy(p, base, buf_size);
            buf_node_data[g][buf] = p;
        }

        if (!ok) {
            for (int g = 1; g < n_groups; g++) {
                if (buf_node_data[g][buf]) {
                    ggml_numa_free(buf_node_data[g][buf], buf_size);
                }
            }
            continue;
        }
    ...
    // set data_numa for each tensor
    int n_set = 0;
    for (auto & [buf, tensors] : buf_tensors) {
        void * buf_base = ggml_backend_buffer_get_base(buf);
        if (!buf_base) continue;

        for (auto * tensor : tensors) {
            size_t offset = (char *)tensor->data - (char *)buf_base;
            void * node_data[GGML_NUMA_MAX_NODES] = { NULL };
            for (int g = 0; g < n_groups; g++) {
                if (buf_node_data[g].count(buf)) {
                    node_data[g] = (char *)buf_node_data[g][buf] + offset;
                }
            }
            ggml_numa_mirror_set(tensor, node_data);
            n_set++;
