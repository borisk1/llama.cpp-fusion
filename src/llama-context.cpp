...
void llama_context::sched_reserve() {
    ...
    if (memory) {
        LLAMA_LOG_INFO("%s: reserving full memory module...\n", __func__);
...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    // Hot-expert cache: read back expert routing decisions after compute
    ...
    if (memory && ubatch.n_tokens > 0) {
        ...
        uint32_t cell_start = ubatch.pos[0];
        uint32_t cell_end   = ubatch.pos[0];
        for (int pi = 0; pi < (int)ubatch.n_tokens; pi++) {
            uint32_t p = ubatch.pos[pi * pos_stride];
            if (p < cell_start) cell_start = p;
            if (p > cell_end)   cell_end   = p;
        }
        uint32_t n_cells = cell_end - cell_start + 1;
        memory->broadcast_thread_copy_kv(cell_start, n_cells);
    }
...
ggml_cgraph * llama_context::graph_reserve(
    ...
    LLAMA_LOG_INFO("%s: reserving graph for ubatch n_tokens=%u n_seqs=%u n_outputs=%u\n", __func__, n_tokens, n_seqs, n_outputs);
...
ggml_status llama_context::graph_compute(
    ...
    // disable hierarchical barrier when multiple backends (CUDA+CPU) present
    // data_numa dispatch still works for per-group memory locality
    if (tp && backend_ptrs.size() > 1 && ggml_threadpool_get_n_groups(tp) > 1) {
...
llama_context * llama_init_from_model(
    ...
    try {
        ...
        // Initialize hot-expert cache profiling (controlled via env vars)
        hot_cache_init(model->hparams.n_layer(), model->hparams.n_expert);
        return ctx;
    } catch (const std::exception & err) {
...
void llama_memory_set_thread_copy_groups(llama_memory_t mem, int n_groups, const int * numa_node_ids) {
    if (!mem) return;
    mem->set_thread_copy_groups(n_groups, numa_node_ids);
}

void llama_memory_broadcast_thread_copy_kv(llama_memory_t mem, uint32_t cell_start, uint32_t n_cells) {
    if (!mem) return;
    mem->broadcast_thread_copy_kv(cell_start, n_cells);
}
