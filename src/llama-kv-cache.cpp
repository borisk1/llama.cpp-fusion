...
void llama_kv_cache::set_thread_copy_groups(int n_groups, const int * numa_node_ids) {
    if (n_groups < 2) return;
    m_n_thread_copy_groups = n_groups;

    for (auto & layer : layers) {
        ggml_tensor * tensors[] = { layer.k, layer.v, nullptr };
        for (int ti = 0; tensors[ti]; ti++) {
            auto * t = tensors[ti];
            if (!t || !t->data || t->view_src) continue;
            ...
            for (int g = 1; g < n_groups; g++) {
                ...
                void * p = ggml_numa_alloc(nbytes, node);
                if (!p) { ok = false; break; }
                memcpy(p, t->data, nbytes);
                node_data[g] = p;
            }

            if (!ok) {
                for (int g = 1; g < n_groups; g++) {
                    if (node_data[g]) ggml_numa_free(node_data[g], nbytes);
                }
                continue;
            }

            ggml_numa_mirror_set(t, node_data);
        }
    }
}

void llama_kv_cache::broadcast_thread_copy_kv(uint32_t cell_start, uint32_t n_cells) {
    int n_groups = m_n_thread_copy_groups;
    if (n_groups < 2 || layers.empty() || n_cells == 0) return;

    for (auto & layer : layers) {
        ggml_tensor * tensors[] = { layer.k, layer.v, nullptr };
        for (int ti = 0; tensors[ti]; ti++) {
            auto * t = tensors[ti];
            if (!t || !t->data || !t->data_numa) continue;
            struct ggml_numa_mirror * m = (struct ggml_numa_mirror *)t->data_numa;

            size_t nbytes = ggml_nbytes(t);
            uint32_t n_cells_total = (uint32_t)t->ne[1];
            if (n_cells_total == 0 || cell_start >= n_cells_total) continue;
            if (cell_start + n_cells > n_cells_total) {
                n_cells = n_cells_total - cell_start;
            }
            size_t bytes_per_cell = nbytes / n_cells_total;
            size_t offset = (size_t)cell_start * bytes_per_cell;
            size_t copy_size = (size_t)n_cells * bytes_per_cell;

            for (int g = 1; g < n_groups; g++) {
                if (m->data[0] && m->data[g]) {
                    memcpy((char *)m->data[g] + offset, (char *)m->data[0] + offset, copy_size);
                }
            }
        }
    }
